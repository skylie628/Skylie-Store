...
export default function CartPage() {
    ...
    const action = useSelector(state => state.cart).action;
    useEffect(()=>{
   ...
   console.log('tat ca la',total)
    ...
    <div>
       ...
       {(cart?.action == actionTypes.DELETE_SUCCESS) && <HeaderNofify style = {{width: '100%'}} severity ='' msg={'Xóa item khỏi giỏ hàng thành công'} ></HeaderNofify>}
        ...
        {
        cart?.cartItems?.length == 0 && action == actionTypes.GET_ALL_SUCCESS && <EmptyCard style = {{margin:isTabletOrMobile?'0px':'30px'}}msg ='Giỏ hàng của bạn đang trống, shopping thôi nào'></EmptyCard>
        }
        ...
        <div className ={styles.totalWrapper} style ={{fontSize:isTabletOrMobile?'16px':'20px',margin: isTabletOrMobile?'20px': ''}}>
            ...
            <Button  label = "Checkout ngay!" color ='black' style ={{marginTop:isTabletOrMobile?'20px':'',padding:'10px', width:  isTabletOrMobile?'200px':''}} onClick ={()=>navigate('/checkout')} ></Button>
        </div>}
        <Suggestion/>
    </div>
  )
